...
(function () {
  ...
  function Wave(canvas) {
    ...
    this.resize();
    this.refreshAccent();
    // The pill's own wave (unlike the header one) sits in a box that changes
    // width when the panel opens. resize() at construction time or at start()
    // catches whatever width the box has *then*, so the backing store stays
    // sized for the collapsed pill and gets visibly stretched once it widens.
